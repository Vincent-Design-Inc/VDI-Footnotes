...
class GitHubUpdater {
     ...
     */
    public function buildPluginDetails(
        array|false|object $result,
        string $action,
        object $args
    ): array|false|object {
        ...
        $pluginBannerSmall = $this->getPluginBannerSmall();
        if ( $pluginBannerSmall ) {
            $result['banners']['low'] = $pluginBannerSmall;
        }

        // If large plugin banner exists, set it
        $pluginBannerLarge = $this->getPluginBannerLarge();
        if ( $pluginBannerLarge ) {
     ...
     */
    public function logPluginDetails(
        object $res,
        string $action,
        object $args
    ): object {
        ...
        $this->logStart( 'logPluginDetails', 'plugins_api_result' );
        $this->logValue( 'Return $res', $res );
        $this->logValue( '$action', $action );
        $this->logValue( '$args', $args );
        $this->logFinish( 'logPluginDetails' );
    ...
    public function checkUpdates(
        ...
        $pluginData['icons'] = array( 'default' => $pluginIcon );
     ...
     */
    public function prepareRequestArgs( array $args, string $url ): array {
        ...
        $this->logStart( 'prepareRequestArgs', 'http_request_args' );
        $this->logValue( 'Return', $args );
        $this->logFinish( 'prepareRequestArgs' );
     ...
     * @return array
     */
    public function moveUpdated( array $result, array $options ): array {
     ...
     * @param string $contents String contents of plugin header fields.
    ...
    private function convertMarkdownHeader( string $line ): array {
        $html = preg_replace_callback(
            '/(#{1,6}) (.+)/',
            function ( $arrMatch ) {
                $size = strlen( $arrMatch[1] );
                return '<h' . $size . '>' . $arrMatch[2] . '</h' . $size . '>';
            },
            $line
        );
    ...
    private function convertMarkdownParagraph( string $line ): array {
        return array( '<p>' . $this->convertInlineMarkdown( $line ) . '</p>' );
    }
    ...
    private function log( string $message, ): void {
        if ( ! $this->enableDebugger || ! WP_DEBUG || ! WP_DEBUG_LOG ) {
			return;
        }

        error_log( '[GitHubUpdater] ' . $message ); // phpcs:ignore
    }

    /**
     * Log when method starts running.
     *
     * @param string $method checkUpdates.
